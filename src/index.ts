...
import Pet from './schemas/Cocina'; // Esquema actualizado a Pet
...
app.post('/post_cocina', async (req, res) => {
  const { nombre, tipoCocina, ingredientes, precio, userId } = req.body;
  console.log("Datos recibidos:", req.body);

  if (!nombre || !tipoCocina || !ingredientes || !precio || !userId) {
    return res.status(400).json({ message: 'Todos los campos son obligatorios.' });
  }

  try {
    const nuevaPet = new Pet({ nombre, tipoCocina, ingredientes, precio, userId});
    await nuevaPet.save();
